...
# 保存先作成
# =========================
session_name = time.strftime("calib_A_%Y%m%d_%H%M%S")
...
# 強制的に DSHOW バックエンドを使用する
# =========================
# カメラ開始
# =========================
def init_camera(index):
    """カメラを開いてすぐに解像度とFPSを下げる関数"""
...
# 1台ずつ順番に起動して帯域を節約する
cap0 = init_camera(CAM0_INDEX)
if cap0 is None:
    raise RuntimeError(f"カメラ0 (Index {CAM0_INDEX}) を開けませんでした。")

cap1 = init_camera(CAM1_INDEX)
if cap1 is None:
    raise RuntimeError(f"カメラ1 (Index {CAM1_INDEX}) を開けませんでした。")
