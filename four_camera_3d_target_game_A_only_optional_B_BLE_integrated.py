...
# True : 黄色が今フレームでLIVE検出され、かつ黄色ゴール内のときだけ点灯
# False: 一時的な見失い時のLAST_USED判定でも点灯を維持する
...
class BleFeedbackController:
    """
    XIAO nRF52840 SenseへBLEで 1/0 を送るためのクラス。
...
def main():
    ...
    try:
        while True:
            ...
            # =========================
            yellow_state = final_states.get("YELLOW")
            yellow_target_ready = (set_a.states["YELLOW"]["target"] is not None) or (set_b is not None and set_b.states["YELLOW"]["target"] is not None)
            ...
            yellow_feedback_on = ready and yellow_target_ready and yellow_inside and yellow_visible
            ...
            ble_text = "YELLOW BLE: ON" if yellow_feedback_on else "YELLOW BLE: OFF"
            ble_color = (0, 255, 255) if yellow_feedback_on else (120, 120, 120)
        ...
        if 'ble_feedback' in locals() and ble_feedback is not None:
            ble_feedback.set_feedback(False)
            time.sleep(0.3)
            ble_feedback.stop()
