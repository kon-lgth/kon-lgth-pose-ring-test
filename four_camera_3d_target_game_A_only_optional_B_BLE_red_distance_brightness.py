...
# distance >= FEEDBACK_MAX_DISTANCE_MM: 白色待機
# distance == 0: 最大赤色
...
# 例: CLEAR_DISTANCE_MM=390なら、780mm以内から赤くなり始める
...
class BleFeedbackController:
    ...
    ※ 可変輝度にするには、Arduino側でも 2-255 を赤色輝度として扱う必要があります。
    """
            ...
            try:
                async with BleakClient(target.address) as client:
                    ...
                    if client.is_connected:
                        try:
                            await client.write_gatt_char(self.char_uuid, bytes([self.STATE_OFF]), response=True)
                        except Exception:
                            pass
...
def main():
    ...
    overall_clear = False
    hold_elapsed = 0.0
    force_ble_state = None  # None=自動, 1=白固定, 2-255=赤輝度固定, 0=消灯固定（動作確認用）
    ...
    try:
        while True:
            ...
            # =========================
            # XIAO BLEフィードバック
            # FEEDBACK_TARGET_COLOR との距離から、赤色LEDの輝度を決める。
            # 近づくほど 2-255 の値が大きくなり、遠いときは 1=白色待機。
            # =========================
            ...
            if REQUIRE_TARGET_LIVE_FOR_LED:
                target_visible = bool(target_state and target_state["source"] in ["A_LIVE", "B_LIVE"])
            else:
                target_visible = bool(target_state and target_state["source"] in ["A_LIVE", "B_LIVE", "LAST_USED"])

            target_distance = target_state["distance"] if target_state else None

            if target_goal_ready and target_visible:
                red_brightness = distance_to_red_brightness(target_distance)
            else:
                red_brightness = None

            if ble_feedback is not None:
                # 動作確認用の強制モード。通常は force_ble_state=None の自動制御。
                if force_ble_state is None:
                    # BLE接続中は白、対象色に近づくほど赤が強くなる
            ...
            cv2.putText(info, "[s] Set Target [c] Reset [r] Reset All [0/1/2] BLE test [a] Auto [q] Quit", (15, 95),
            ...
            if force_ble_state is not None:
                ...
                ble_text = f"BLE LED: SEND 2 {FEEDBACK_TARGET_COLOR} GOAL"
                ble_color = (0, 255, 255)
            ...
            if key == ord("2"):
                force_ble_state = BleFeedbackController.STATE_GOAL
                print("[BLE TEST] force GOAL COLOR: send 2")
            if key == ord("a"):
