...
# 設定
# =========================
CALIB_FILE = r"calibration_images\calib_20260501_151111\stereo_calibration_result.npz"
...
# XIAO BLE LED設定
# =========================
ENABLE_XIAO_BLE_LED = True

BLE_DEVICE_NAME = "PoseRing_YELLOW"
BLE_LED_CHAR_UUID = "19B10001-E8F2-537E-4F6C-D104768A1214"

# True : 黄色がカメラで今見えているときだけLED点灯
# False: 見失い時のLAST座標でもLED判定に使う
REQUIRE_YELLOW_LIVE = False
...
# 一旦、黄色のみを検出・判定する
# 元の4色版: COLOR_ORDER = ["RED", "YELLOW", "BLUE", "GREEN"]
COLOR_ORDER = ["YELLOW"]
...
class BleLedController:
    """
    XIAO nRF52840 Sense にBLEで 1/0 を送るためのクラス。
    ...
    def __init__(self, device_name, char_uuid, scan_timeout=8.0):
        ...
        self.requested_value = None  # 最後に要求された値 0/1
    ...
    def set_led(self, on):
        """
        LED状態を要求する。
        on=True なら 1、on=False なら 0 をXIAOへ送る。
        同じ値を連続して送らない。
        """
...
print("======================================")
print("Yellow Only 3D Target Game BLE を開始")
print("q / Esc : 終了")
print("Enter   : 現在の黄色マーカー位置を原点(0,0,0)に設定")
print("s       : 現在の黄色3D座標を目標として保存")
print("c       : 黄色の目標座標とCLEAR状態をリセット")
print("r       : 原点・目標・CLEAR状態をすべてリセット")
print("※ 色を見失った場合は、最後に検知した3D座標を使用します")
print(f"※ 黄色が目標から {CLEAR_DISTANCE_MM:.0f} mm 以内を {HOLD_TIME_SEC:.1f} 秒キープするとCLEAR")
print("======================================")
...
if ENABLE_XIAO_BLE_LED:
    ble_led = BleLedController(
        BLE_DEVICE_NAME,
        BLE_LED_CHAR_UUID
    )
    ble_led.start()
...
while True:
    ...
    # ======================================
    # XIAO LED制御：黄色がゴール地点に入っているときだけ点灯
    # ======================================
    yellow_target_set = states["YELLOW"]["target_3d"] is not None
    yellow_inside = states["YELLOW"]["inside"]

    if REQUIRE_YELLOW_LIVE:
        yellow_visible = states["YELLOW"]["source"] == "LIVE"
    else:
        yellow_visible = states["YELLOW"]["source"] in ["LIVE", "LAST"]

    yellow_led_on = yellow_target_set and yellow_inside and yellow_visible

    if ble_led is not None:
        ble_led.set_led(yellow_led_on)
    ...
    if overall_clear:
        ...
        overall_text = "NO YELLOW GOAL | s: set yellow goal"
    ...
    cv2.putText(info, "YELLOW ONLY 3D TARGET GAME BLE", (20, 35),
    ...
    cv2.putText(info, "s: set yellow target | c: reset target | q/Esc: quit | LOST => use LAST 3D point", (20, 100),
    ...
    cv2.imshow("Yellow Only 3D Target Game BLE", display)
    ...
    if key == 13 or key == 10:
        yellow_point = states["YELLOW"]["current_point_3d"]
        if yellow_point is not None:
            origin_3d = yellow_point.copy()
            for name in COLOR_ORDER:
            ...
            all_inside_start_time = None
            overall_clear = False
            print(f"原点を設定しました: X={origin_3d[0]:.3f}, Y={origin_3d[1]:.3f}, Z={origin_3d[2]:.3f}  source={states['YELLOW']['source']}")
            print("黄色の目標座標はリセットされました。")
        else:
            print("黄色マーカーの3D座標がないため、原点を設定できません。")

    # sキーで現在の黄色相対座標を目標として保存
    if key == ord("s"):
        missing = [name for name in COLOR_ORDER if states[name]["current_relative_3d"] is None]
        if missing:
            print("黄色の3D座標がないため、目標を設定できません。")
            ...
            if any(states[name]["source"] == "LAST" for name in COLOR_ORDER):
                print("注意: 黄色は見失い中だったため、最後に検知した座標を目標として保存しました。")
    ...
    if key == ord("c"):
        ...
        all_inside_start_time = None
        overall_clear = False
        print("黄色の目標座標とCLEAR状態をリセットしました。")
    ...
    if key == ord("r"):
        ...
        all_inside_start_time = None
        overall_clear = False
        print("原点設定・黄色の目標座標・CLEAR状態をリセットしました。")


# 終了時にXIAOのLEDを消してBLE制御を停止
if ble_led is not None:
    ble_led.set_led(False)
    time.sleep(0.3)
    ble_led.stop()
