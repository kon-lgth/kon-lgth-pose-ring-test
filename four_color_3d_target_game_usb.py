...
# XIAO LED設定
# =========================
ENABLE_XIAO_LED = True
SERIAL_PORT = "COM3"
SERIAL_BAUD = 115200

# True: 黄色がカメラで今見えている時だけLED点灯
...
if ENABLE_XIAO_LED:
    try:
        ...
        time.sleep(2.0)
        print(f"XIAOに接続しました: {SERIAL_PORT}")
    except Exception as e:
        print("XIAOに接続できませんでした。LED制御なしで続行します。")
        print(e)
        ser = None


def send_xiao_led(on):
    """
    XIAOへLED命令を送る。
    on=True なら '1'
    on=Falseなら '0'
...
if ser is not None:
    try:
        ser.write(b"0")
        ser.close()
        print("XIAOのLEDを消灯してシリアル接続を閉じました。")
    except Exception as e:
        print("XIAO終了処理でエラー:", e)