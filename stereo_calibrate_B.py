...
SQUARE_SIZE = 51.0      # 1マスの一辺の長さ(mm)。自分の印刷物に合わせる
...
objpoints = []   # 3D座標
imgpoints0 = []  # Bセット cam0 の2D座標
imgpoints1 = []  # Bセット cam1 の2D座標
...
print("======================================")
print("B SET Stereo calibration start")
...
for f0, f1 in zip(images0, images1):
    ...
    if ret0 and ret1:
        ...
        print(
            f"SKIP : {os.path.basename(f0)} / {os.path.basename(f1)} "
...
if valid_count < 8:
    raise RuntimeError(
        ...
        "最低でも8〜10ペア以上、できれば15〜20ペア以上ほしいです。"
    )
...
print(f"\nEstimated baseline length = {baseline:.3f} mm")

print("\n======================================")
print("Bセット用キャリブレーション完了")
print("次に使うBセット用CALIB_FILEは以下です:")
print(OUTPUT_FILE)
print("======================================")