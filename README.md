...
calibration_images フォルダに、キャリブレーション用の画像ペアが保存されます。
...
緑色マーカーの3次元座標が計算・表示されます。

事前に変更が必要な箇所
カメラインデックス

使用する環境によってカメラ番号が異なるため、必要に応じて変更してください。

CAM0_INDEX = 0
CAM1_INDEX = 1
セッションフォルダ

キャリブレーション画像が保存されたフォルダを指定してください。

SESSION_DIR = r"calibration_images\xxxx"
キャリブレーション結果ファイル

生成された .npz ファイルのパスを指定してください。

CALIB_FILE = r"calibration_images\xxxx\stereo_calibration_result.npz"
注意
キャリブレーション後はカメラ位置を動かさないでください
チェッカーボードは様々な角度・位置で撮影してください
