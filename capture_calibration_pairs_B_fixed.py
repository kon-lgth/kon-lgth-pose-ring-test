...
# カメラごとに開き方を分ける
# Index 0 は DSHOW で開けなかったため DEFAULT にする
CAM0_BACKEND = "DSHOW"
CAM1_BACKEND = "DEFAULT"

PATTERN_SIZE = (9, 6)           # チェッカーボード内側の交点数
SQUARE_SIZE = 51.0              # チェッカーボード1マスの実寸(mm) ※記録用
...
with open(os.path.join(root_dir, "session_info.txt"), "w", encoding="utf-8") as f:
    ...
    f.write(f"SQUARE_SIZE={SQUARE_SIZE}\n")
    ...
    f.write(f"CAM0_BACKEND={CAM0_BACKEND}\n")
    f.write(f"CAM1_BACKEND={CAM1_BACKEND}\n")
...
def open_camera(index, backend_mode):
    """
    カメラごとに開き方を変える。
    DSHOWで開けるカメラはDSHOW、開けないカメラはDEFAULTを使う。
    """
...
cap0 = open_camera(CAM0_INDEX, CAM0_BACKEND)
cap1 = open_camera(CAM1_INDEX, CAM1_BACKEND)
...
print(f"CAM0_INDEX: {CAM0_INDEX} | BACKEND: {CAM0_BACKEND}")
print(f"CAM1_INDEX: {CAM1_INDEX} | BACKEND: {CAM1_BACKEND}")
print(f"PATTERN_SIZE: {PATTERN_SIZE}")
print(f"SQUARE_SIZE: {SQUARE_SIZE} mm")
...
while True:
    ...
    cv2.putText(
        disp0,
        f"B Cam0 | Index {CAM0_INDEX} | {CAM0_BACKEND} | found={found0}",
        (10, 30),
    ...
    cv2.putText(
        disp1,
        f"B Cam1 | Index {CAM1_INDEX} | {CAM1_BACKEND} | found={found1}",
        (10, 30),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.65,
