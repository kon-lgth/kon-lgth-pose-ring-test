...
# BLE goal occupancy hysteresis.
# When one color enters a goal, that goal is hidden from the other colors for
# BLE distance feedback. The owner keeps the claim until it moves outside
# clear_dist * this multiplier. This avoids flicker when coordinates jitter.
DEFAULT_GOAL_OCCUPANCY_RELEASE_MULTIPLIER = 1.35
...
class GameEngine:
    ...
    def _apply_settings(self, settings):
        ...
        release_multiplier = settings.get(
            "goal_occupancy_release_multiplier",
            os.getenv("POSERING_GOAL_OCCUPANCY_RELEASE_MULTIPLIER", DEFAULT_GOAL_OCCUPANCY_RELEASE_MULTIPLIER),
        )
        try:
            release_multiplier = float(release_multiplier)
        except (TypeError, ValueError):
            release_multiplier = DEFAULT_GOAL_OCCUPANCY_RELEASE_MULTIPLIER
        release_multiplier = max(1.0, release_multiplier)
        self._goal_occupancy_release_dist = self._clear_dist * release_multiplier
    ...
    def _nearest_target_ble_states(self, core, target_slots, current_by_color, raw_final_states=None, target_claims=None):
        """Independent BLE feedback target selection with goal occupancy.

        Game clear judging stays color-agnostic and uses _color_agnostic_colors().
        BLE feedback is still calculated per physical ring, but occupied goals
        are hidden from other colors:

        - If RED owns goal A, RED can keep seeing goal A and blink at 250.
        - YELLOW/BLUE/GREEN ignore goal A and are guided to the next nearest
          unoccupied goal.
        - This prevents multiple rings from blinking for the same goal while
          preserving color-agnostic gameplay.
        """
        states = {}
        max_distance = max(self._clear_dist * 3.0, 1.0)
        claims = target_claims or {}
        ...
        for color in COLOR_ORDER:
            ...
            for target_index, slot in enumerate(target_slots or []):
                claim_owner = claims.get(target_index)
                # Occupied goals are invisible to other colors. The owner can
                # still see its own goal and keep the 250 blink.
                if claim_owner not in (None, color):
                    continue
            ...
            if nearest is not None:
                ...
                inside = bool(distance <= self._clear_dist and claims.get(nearest_index) in (None, color))
                ...
                states[color] = {
                    ...
                    "claimed_by": claims.get(nearest_index) if nearest_index is not None else None,
                    "occupied_target_indexes": {str(i + 1): c for i, c in claims.items()},
                }
            else:
    ...
    def _claim_color_target_assignment(self, distance_matrix, target_claims):
        """Assign colors to goals using persistent goal occupancy.

        The game remains color-agnostic, but each physical goal should only
        produce the 250 blink for one ring at a time. Once a color claims a
        goal, that goal is hidden from the other colors when their nearest-goal
        BLE feedback is calculated.

        Example:
            RED claims goal A.
            YELLOW is also physically near goal A.
            For YELLOW, goal A is ignored, so YELLOW is guided toward the next
            nearest unoccupied goal instead of blinking on goal A.

        Claims are released with hysteresis: the owner must move beyond
        _goal_occupancy_release_dist, not just barely outside clear_dist.
        """
        claims = {}
        ...
        assignment = {}
        used_colors = set()
        release_distance = float(getattr(self, "_goal_occupancy_release_dist", self._clear_dist * 1.35))

        # Keep existing claims first. This makes the earlier entrant remain the
        # owner while it stays near the goal, so later entrants cannot steal it.
        for target_index, color in list(claims.items()):
            ...
            # Reserve this color while the claim is alive. This avoids the same
            # ring claiming another nearby goal just because it jittered slightly
            # outside clear_dist. It will still receive normal distance feedback
            # unless it is actually inside clear_dist.
            used_colors.add(color)
            if distance <= self._clear_dist:
                assignment[color] = target_index

        # Free goals can be claimed by the nearest unused color inside the clear
        # range. Already-claimed goals are skipped, so other colors effectively
        # cannot see occupied goals.
        candidates = []
    ...
    def _update_multi_ble_distance_feedback(self, core, ctx, feedbacks, final_states):
        ...
        the already computed final_states/colors data. That data already applies
        goal occupancy, so a goal occupied by one color is hidden from the other
        colors and they are guided toward the next nearest unoccupied goal.
        """
        if not feedbacks:
            with self._lock:
        ...
        for color, controller in feedbacks.items():
            ...
            rings[color] = {
                ...
                "matched_target_index": fs.get("matched_target_index"),
                "claimed_by": fs.get("claimed_by"),
                "occupied_target_indexes": fs.get("occupied_target_indexes", {}),
                "value": int(value),
                "reason": reason,
            }
        ...
        try:
            while self._running:
                ...
                if ctx.get("target_slots"):
                    ...
                    # BLE feedback is calculated per physical ring, but it uses
                    # the same persistent goal claims. A goal occupied by one color
                    # is hidden from the other colors, so later rings are guided
                    # toward the next nearest unoccupied goal instead of blinking
                    # on the same occupied goal.
                    ble_states = self._nearest_target_ble_states(
                        core,
                        target_slots,
                        current_by_color,
                        raw_final_states,
                        ctx.get("target_claims", {}),
                    )
                else:
