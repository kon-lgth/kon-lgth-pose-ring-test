...
def _vec_to_list(v):
    ...
    if p is None:
        return None
    return [float(x) for x in p.reshape(-1).tolist()]




def _safe_point(v):
    """Return a finite 3D numpy point or None.

    Some live/target dictionaries may temporarily contain None inside
    a list, for example [x, y, None].  Converting that directly can
    later crash subtraction with: float - NoneType.
    """
    if v is None:
        return None
    try:
...
class GameEngine:
        ...
        for color in COLOR_ORDER:
            ...
            for set_name, set_points in (sets or {}).items():
                ...
                if not isinstance(set_points, dict):
                    continue
                point = _safe_point(set_points.get(color))
                if point is not None:
                    points[set_name] = point
            if points:
    ...
    def _target_slots_from_current_targets(self, set_a=None, set_b=None):
        ...
        for color in COLOR_ORDER:
            ...
            if set_a is not None:
                a_target = _safe_point(set_a.states[color].get("target"))
                if a_target is not None:
                    points["A"] = a_target
            if set_b is not None:
                b_target = _safe_point(set_b.states[color].get("target"))
    ...
    def _current_points_from_sets(self, set_a=None, set_b=None):
        ...
        for color in COLOR_ORDER:
            if set_a is not None:
                ...
                point = _safe_point(st.get("live_point") if st.get("live") else st.get("current_point"))
                if point is not None:
                    current[color]["A"] = {
                        "point": point,
    ...
    def _current_points_from_color_state(self, colors):
        current = {color: {} for color in COLOR_ORDER}
        for color in COLOR_ORDER:
            point = _safe_point((colors or {}).get(color, {}).get("current"))
            if point is not None:
                current[color]["SIM"] = {
                    "point": point,
                    "source": "SIM",
    ...
    def _target_distance_for_color(self, color_current, slot):
        ...
        for set_name, target in slot.get("points", {}).items():
            # A/Bどちらか片方の目標点が無い場合や、
            # 途中でNoneが混ざった場合にLive処理全体が落ちないようにする。
            if target is None:
                continue
            ...
            current_point = cur.get("point")
            if current_point is None:
                continue

            current_point = _safe_point(current_point)
            target_point = _safe_point(target)
            if current_point is None or target_point is None:
                continue

            distance = float(np.linalg.norm(current_point - target_point))
    ...
    def _process_commands(self, commands, ctx, set_a=None, set_b=None):
        for cmd, data in commands:
            if cmd == "start_game":
                ...
                ctx["all_inside_t"] = None
                ctx["pose_result"] = None
                ctx["clear_ble_sent"] = False
                ctx["clear_ble_t"] = None
    ...
    def _ble_device_names(self):
        """
        4台リング用BLE名。
        必要なら環境変数で上書きできる。
          POSERING_BLE_RED=PoseRing_RED
          POSERING_BLE_YELLOW=PoseRing_YELLOW
          POSERING_BLE_BLUE=PoseRing_BLUE
          POSERING_BLE_GREEN=PoseRing_GREEN
        """
        return {
    ...
    def _start_ble_feedback(self, core):
        """
        BLEフィードバックを開始する。
        デフォルトは4台同時制御。
        POSERING_BLE_MODE=single を指定した場合だけ旧1台制御に戻せる。
        """
        mode = os.getenv("POSERING_BLE_MODE", self._settings.get("ble_mode", "multi")).strip().lower()
        char_uuid = self._settings.get("ble_char_uuid", core.BLE_LED_CHAR_UUID)

        if mode == "single":
            controller = core.BleFeedbackController(
        ...
        controllers = {}
        for color, device_name in self._ble_device_names().items():
            controller = core.BleFeedbackController(device_name, char_uuid)
            controller.start()
            controllers[color] = controller
        return controllers

    def _stop_ble_feedback(self, core, ble_feedback):
        if ble_feedback is None:
            return
        if isinstance(ble_feedback, dict):
            for controller in ble_feedback.values():
    ...
    def _state_value_for_color(self, core, ctx, color, state, stage_visible):
        """
        色非固定判定を維持したまま、BLE送信先は色リングごとに固定する。
        - 見えていないリング: 0
        - 見えているがゲーム中でない/距離がない: 1
        - ゲーム中: 距離に応じて2〜249、ゴール内は250
        - POSE_CLEAR後: 252
        """
        ...
        if not stage_visible:
            return core.BleFeedbackController.STATE_OFF

        if not (
            ctx.get("game_state") == GameState.PLAYING
            and ctx.get("targets_set", False)
            and state
            and state.get("distance") is not None
        ):
            return core.BleFeedbackController.STATE_CONNECTED_WHITE

        if state.get("inside"):
            return 250

        brightness = core.distance_to_red_brightness(state.get("distance"))
        if brightness is None:
            return core.BleFeedbackController.STATE_CONNECTED_WHITE
        return max(core.BleFeedbackController.MIN_RED_BRIGHTNESS_VALUE, min(249, int(brightness)))

    def _update_ble_single(self, core, ctx, final_states, ble_feedback, last_target_live_time):
        if ble_feedback is None:
        ...
        value = self._state_value_for_color(core, ctx, target_color, target_state, stage_visible)
    ...
    def _update_ble_multi(self, core, ctx, final_states, ble_feedback, last_live_times):
        if ble_feedback is None:
            self._ble_status = {"enabled": False}
            return last_live_times

        now = time.time()
        last_live_times = dict(last_live_times or {})
        status_by_color = {}

        # POSE_CLEARに入った直後だけREDへ251を送り、クリア音を鳴らす。
        # その後は全リングへ252を送り、自分の色で最大点灯を維持する。
        clear_now = ctx.get("game_state") == GameState.POSE_CLEAR
        if clear_now and not ctx.get("clear_ble_sent", False):
            red = ble_feedback.get("RED")
            if red is not None:
                red.set_state(251)
            ctx["clear_ble_sent"] = True
            ctx["clear_ble_t"] = now

        for color in COLOR_ORDER:
            controller = ble_feedback.get(color)
            state = final_states.get(color, {})
            live = bool(state and state.get("source") in ["A_LIVE", "B_LIVE"])

            if live:
                last_live_times[color] = now
                stage_visible = True
                lost_age = 0.0
            elif color not in last_live_times:
                stage_visible = False
                lost_age = None
            else:
                lost_age = now - last_live_times[color]
                stage_visible = lost_age <= core.STAGE_LOST_GRACE_SEC

            value = self._state_value_for_color(core, ctx, color, state, stage_visible)

            # REDは251送信直後に252で上書きしないよう、短時間だけ251を維持する。
            if clear_now and color == "RED" and ctx.get("clear_ble_sent", False):
                clear_age = now - (ctx.get("clear_ble_t") or now)
                if clear_age < 0.5:
                    value = 251
            ...
            status_by_color[color] = {
                "connected": bool(controller and controller.is_connected),
                "device": getattr(controller, "device_name", self._ble_device_names().get(color)),
                "value": int(value),
                "stage_visible": bool(stage_visible),
                "lost_age": lost_age,
                "source": state.get("source", "NO DATA") if state else "NO DATA",
                "distance": state.get("distance") if state else None,
                "inside": bool(state.get("inside", False)) if state else False,
            }

        self._ble_status = {
            "enabled": True,
            "mode": "multi",
            "connected": all(item["connected"] for item in status_by_color.values()),
            "colors": status_by_color,
        }
        return last_live_times

    def _update_ble(self, core, ctx, final_states, ble_feedback, last_live_times):
        if ble_feedback is None:
            self._ble_status = {"enabled": False}
            return last_live_times
        if isinstance(ble_feedback, dict):
            return self._update_ble_multi(core, ctx, final_states, ble_feedback, last_live_times)
        return self._update_ble_single(core, ctx, final_states, ble_feedback, last_live_times)
    ...
    def _run_live(self):
        ...
        if bool(self._settings.get("ble_enabled", core.ENABLE_XIAO_BLE)):
            ble_feedback = self._start_ble_feedback(core)

        ctx = self._make_context()
        last_used = {color: None for color in COLOR_ORDER}
        last_target_live_time = {color: None for color in COLOR_ORDER}
        ...
        try:
                ...
                if ctx.get("target_slots"):
                    ...
                    final_states = {
                        color: {
                            ...
                            "current": _safe_point(colors[color].get("current")),
                            "target": _safe_point(colors[color].get("target")),
            ...
            self._stop_ble_feedback(core, ble_feedback)
            set_a.release()
            if set_b is not None:
                set_b.release()
