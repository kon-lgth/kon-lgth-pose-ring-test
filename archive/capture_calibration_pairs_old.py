...
# 設定
# =========================
CAM0_INDEX = 0                  # ノートPC内蔵カメラ
CAM1_INDEX = 1                  # iPhone (DroidCam)
...
while True:
    ...
    # 🚨 ここにあった cv2.flip を削除（生画像のまま処理する）🚨
    ...
    if elapsed < START_DELAY_SEC:
        ...
        if found0 and found1:
            enough_interval = (now - last_save_time) >= CAPTURE_INTERVAL_SEC
            ...
            if saved_count < TARGET_PAIRS and enough_interval and enough_change:
                ...
                if saved_count >= TARGET_PAIRS:
                    ...
                    wait_sec = CAPTURE_INTERVAL_SEC - (now - last_save_time)
                    message = f"Wait next capture: {wait_sec:.1f} sec"
                elif not enough_change:
    ...
    # ======================================
    # 画面表示用の処理（ここで初めて反転し、その後に文字を描画する）
    # ======================================
