...
A_CALIB_FILE = r"calibration_images\calib_A_20260506_154254\stereo_calibration_result.npz"
# Bセット用キャリブレーション結果
...
# Bセットを使うかどうか
# False にすると、Bカメラを接続していなくてもAセットだけで起動します。
USE_B_SET = False
...
# True : 黄色が今フレームでLIVE検出され、かつ黄色ゴール内のときだけ黄色点灯
# False: 一時的な見失い時のLAST_USED判定でも黄色点灯を維持する
REQUIRE_YELLOW_LIVE_FOR_LED = False
...
# 判定設定
CLEAR_DISTANCE_MM = 390.0   # 目標からこの距離以内ならOK（51mmキャリブレーション用）
HOLD_TIME_SEC = 2.0         # 4色すべてOKをこの秒数キープしたらCLEAR

LOST_HOLD_SEC = 0.7         # 色を見失った時、最後の座標を使い続ける最大時間
...
class BleFeedbackController:
    """
    XIAO nRF52840 SenseへBLEで状態値を送るためのクラス。
    ...
    def set_feedback(self, yellow_goal_on):
        """
        yellow_goal_on=True なら黄色、FalseならBLE接続中の白を要求する。
        """
        self.set_state(self.STATE_YELLOW_GOAL if yellow_goal_on else self.STATE_CONNECTED_WHITE)
            ...
            try:
                async with BleakClient(target.address) as client:
                    ...
                    if client.is_connected:
                        try:
                            await client.write_gatt_char(self.char_uuid, bytes([self.STATE_OFF]), response=False)
                        except Exception:
                            pass
    ...
    def __init__(self, name, calib_file, cam0_index, cam1_index, backend_mode):
        ...
        for color in COLOR_ORDER:
            self.states[color] = {
                ...
                "last_point": None,      # このセットで最後にLIVE取得した3D座標
    ...
    def read_and_process(self):
        ...
        for color in COLOR_ORDER:
            ...
            if data_3d is not None:
                ...
                cv2.line(self.out0, (0, data_3d["cy0"]), (self.out0.shape[1] - 1, data_3d["cy0"]), cfg["box_color"], 1)
                cv2.line(self.out1, (0, data_3d["cy1"]), (self.out1.shape[1] - 1, data_3d["cy1"]), cfg["box_color"], 1)
            else:
                st["live"] = False
                st["live_point"] = None
                st["y_diff"] = None
...
def main():
    ...
    try:
        while True:
            ...
            # =========================
            # XIAO BLEフィードバック
            # 黄色が目標範囲内に入ったら、外付けNeoPixel/LEDを点灯
            # =========================
            yellow_state = final_states.get("RED")
            yellow_target_ready = (set_a.states["RED"]["target"] is not None) or (set_b is not None and set_b.states["RED"]["target"] is not None)
            yellow_inside = bool(yellow_state and yellow_state["inside"])

            if REQUIRE_YELLOW_LIVE_FOR_LED:
                yellow_visible = bool(yellow_state and yellow_state["source"] in ["A_LIVE", "B_LIVE"])
            else:
                yellow_visible = bool(yellow_state and yellow_state["source"] in ["A_LIVE", "B_LIVE", "LAST_USED"])

            yellow_feedback_on = yellow_target_ready and yellow_inside and yellow_visible

            if ble_feedback is not None:
                # BLE接続中は白、黄色がゴール判定に入ったら黄色
                ble_feedback.set_feedback(yellow_feedback_on)
            ...
            cv2.putText(row_a, "A SET", (10, row_a.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
            
            cv2.rectangle(row_b, (5, row_b.shape[0] - 30), (70, row_b.shape[0] - 5), (0, 0, 0), -1)
            cv2.putText(row_b, "B SET", (10, row_b.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
            ...
            if yellow_feedback_on:
                ble_text = "BLE LED: YELLOW GOAL"
                ble_color = (0, 255, 255)
            elif ble_feedback is not None and ble_feedback.is_connected:
                ble_text = "BLE LED: CONNECTED WHITE"
                ble_color = (255, 255, 255)
            else:
                ble_text = "BLE LED: SEARCHING/OFF"
                ble_color = (120, 120, 120)
            cv2.putText(info, ble_text, (420, 430),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, ble_color, 2)
        ...
        if 'ble_feedback' in locals() and ble_feedback is not None:
