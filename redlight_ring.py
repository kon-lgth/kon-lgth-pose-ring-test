...
def main():
    ...
    try:
        while True:
            ...
            # BLEへ送る値を決める。
            # 重要: ゴール判定内(target_inside=True)のときは必ず255を送る。
            # Arduino側では 250以上を「ゴール中」として扱うため、
            # ここで255を送り続けることで、点滅と3秒カウントが安定して動く。
            if target_goal_ready and target_visible and target_distance is not None:
                if target_inside:
                    red_brightness = FEEDBACK_MAX_RED_BRIGHTNESS  # ゴール内は必ず255
                else:
            ...
            # BLE送信の動作確認用
            # 0: 消灯固定, 1: 白固定, 2: 赤最大固定, a: 自動制御に戻す
            if key == ord("0"):
